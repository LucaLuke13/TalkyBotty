...
class telegram:
    ...
    def split_message_to_chunks(self, input_string):
        ...
        for line in lines:
            ...
            new_chunk_size = len(line) + temp_chunk_size   # Add  1 for the newline character if not the first line
            
            # If adding this line would exceed the maximum chunk size, finalize the current chunk and start a new one
    ...
    def delete_file(self, file_path):
        try:
            ...
            logger.error(f"You do not have permission to delete the file '{file_path}'.")
    ...
    def transcribe(self, wav_file):
        ...
        command = [whisper_path, "--model", self.modelpath, "--language", os.getenv('TRANSLATELANG'), "--file", "--translate", wav_file]
        print(command)
